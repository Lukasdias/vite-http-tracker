...
A developer-experience tool that captures the HTTP requests a running frontend app makes (the traffic you'd see in the browser Network panel) and renders them as a **time-ordered**, interactive graph on a separate port. It runs as a sidecar — one local Node server on `:4000`, one WebSocket-fed agent injected into the tracked app — plus a React dashboard. The Vite plugin starts and stops the sidecar automatically during `vite dev`.
...
| `packages/server` | Node HTTP + `ws`. Ingests, stores, broadcasts, and serves the UI. |
| `packages/ui` | React 19 dashboard. React Flow timeline, filters, inspector, dedup/batch logic. |
| `packages/plugin` | Vite plugin `viteHttpTracker()`. Starts the server, auto-injects the agent, and detects Strict Mode at build time. |
...
| `packages/ui/dist` | Workspace build output for the dashboard. |
| `dist/ui/dist` | Published dashboard bundle copied into the package and served by the server (SPA fallback). |
...
bun run --cwd apps/react-app dev   # run the sample app and tracker
```
...
flowchart LR
  ...
  S["server: Node HTTP + ws (127.0.0.1:4000)"]
...
- **Server** (`packages/server`): `store.ts` (byte-LRU), `server.ts` (Node HTTP + `ws`, static serving of published `ui/dist` with `public/` as dev fallback). No React, no UI logic. Binds to `127.0.0.1`. Auth token required on every write and every WS upgrade.
...
- **Plugin** (`packages/plugin`): `viteHttpTracker()` development plugin. `apply: "serve"`. Starts the local tracker in `configureServer`, reports its dashboard URL in the Vite log, closes it with Vite, injects a virtual module (`virtual:vite-http-tracker/agent`) into `index.html` via `transformIndexHtml`, and detects whether the app uses Strict Mode by scanning `src/` in `configResolved`.
...
- **Import with `.js` extension** in TS (e.g. `./graph.js`) for Node/Vite and Bun test resolution.
...
- **Package exports** point at `.ts` sources in the workspace and compiled `.js`/`.d.ts` files in the published package; the publish build compiles the libraries.
...
- **New server behavior (endpoint / store):** `server/src/server.ts` / `store.ts`; mirror in `server.test.ts` / `e2e.test.ts`.
...
- The plugin owns the server lifecycle: `configureServer` starts it, and Vite's `httpServer` close event shuts it down. `autoInject: false` disables both injection and automatic startup.
